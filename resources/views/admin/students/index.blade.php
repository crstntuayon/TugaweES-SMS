...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="{{ route('admin.students.create') }}"
       class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow-lg transition duration-200 hover:scale-105">
        Add Student
    </a>
</div>
    ...
    <!-- Students Table -->
    <div class="bg-white rounded-2xl shadow overflow-hidden animate-fadeIn">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-indigo-50 transition duration-200">
                    <td class="px-6 py-4 text-gray-700 font-medium">{{ $student->first_name }} {{ $student->last_name }}</td>
                    <td class="px-6 py-4 text-gray-600">{{ $student->section->name ?? 'Not Assigned' }}</td>
        ...
        @if($students->isEmpty())
            <p class="p-6 text-center text-gray-500">No students found.</p>
        @endif
    </div>
